...
class AdamUCB(Optimizer):
    r"""Implements AdamUCB algorithm. Biased gradient for eta !=0.
    Biases the updates along directions of the loss surface that agree across
    mini-batches.
    ...
    Arguments:
        ...
        eta: The confidence hyperparameter that the bias towards the variance-reduction
            objective
    ...
    def __init__(self, params, lr=1e-3, eta=0.01, betas=(0.9, 0.999), eps=1e-8,
        ...
        super(AdamUCB, self).__init__(params, defaults)

    def __setstate__(self, state):
        super(AdamUCB, self).__setstate__(state)
    ...
    def step(self, closure):
        ...
        if closure is not None:
            ...
            raise RuntimeError('AdamUCB needs closure.')
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                grad = p.grad.data
                if grad.is_sparse:
                    raise RuntimeError('AdamUCB does not support sparse gradients, please consider SparseAdam instead')
